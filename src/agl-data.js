...
// Generated: 2026-01-02T19:05:24.659Z
//
// This file contains glyph name to Unicode mappings from the Adobe Glyph List (AGL).
// AGL provides standard glyph names for characters in the Adobe Standard Encoding,
// Mac Roman, Windows ANSI, and other legacy font encodings.
...
(function() {
  ...
  // Reverse mapping: Unicode → Glyph Name
  // Build from AGL first, then overwrite with AGLFN (canonical/preferred names)
  const UNICODE_TO_GLYPH_NAME = Object.fromEntries(
    Object.entries(AGL_MAPPINGS).map(([k, v]) => [v, k])
  );
  for (const [name, ch] of Object.entries(AGLFN_MAPPINGS)) {
    UNICODE_TO_GLYPH_NAME[ch] = name;
  }

  // Expose to window
  if (typeof window !== 'undefined') {
    window.AGL_MAPPINGS = AGL_MAPPINGS;
    window.AGLFN_MAPPINGS = AGLFN_MAPPINGS;
    window.UNICODE_TO_GLYPH_NAME = UNICODE_TO_GLYPH_NAME;
  }
})();
